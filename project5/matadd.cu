...
__global__ void addKernel(const float *input1, const float *input2,
                          float *output, const float alpha, const float beta,
                          size_t len) {
  int i = blockDim.x * blockIdx.x + threadIdx.x;
  if (i < len)
    output[i] = input1[i] + alpha * input2[i] + beta;
}
...
bool addGPU(const Matrix *pMat1, const Matrix *pMat2, Matrix *pMat3,
  ...
  cudaMemcpy(pMat2->data_device, pMat2->data, sizeof(float) * len,
             cudaMemcpyHostToDevice);
  addKernel<<<(len + 255) / 256, 256>>>(pMat1->data_device, pMat2->data_device,
                                        pMat3->data_device, alpha, beta, len);
...
int main() {
  ...
  TIME_END(addCPU)
  printf("  Result = [%.1f, ..., %.1f]\n", pMat3->data[0],
         pMat3->data[pMat3->rows * pMat3->cols - 1]);

  TIME_START
  addGPU(pMat1, pMat2, pMat3, 5.0f, 3.0f);
  TIME_END(addGPU)
  printf("  Result = [%.1f, ..., %.1f]\n", pMat3->data[0],
         pMat3->data[pMat3->rows * pMat3->cols - 1]);
